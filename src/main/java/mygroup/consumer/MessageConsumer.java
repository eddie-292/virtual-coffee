...
@Slf4j
@Component
@RabbitListener(queues = QueueConstant.User_Star_Msg)
public class MessageConsumer {
    ...
    public void process(String jsonData) {
        ...
        String type = map.get("type").toString(); // add or sub
        double amount = Double.valueOf(map.get("amount").toString());
        String description = map.get("description").toString();
        Date date = new Date();

        UserStarRecords userStarRecords = new UserStarRecords();
        userStarRecords.setUserId(userId);
        userStarRecords.setTime(date);
        userStarRecords.setType(type);
        userStarRecords.setMsgType(msgType);
        userStarRecords.setAmount(BigDecimal.valueOf(amount));
        userStarRecords.setBalance(BigDecimal.ZERO);
        userStarRecords.setDescription(description);
        userStarRecordsMapper.insert(userStarRecords);
