...
@Slf4j
@Component
@RabbitListener(queues = QueueConstant.Code_Snippet_Delete)
public class FeedsDelConsumer {
    ...
    public void process(String jsonData) {
        ...
        criteria.and("userId").is(Long.valueOf(userId));
        criteria.and("batteryLevel").lte(0);

        Query query = new Query(criteria);
        DeleteResult deleteResult = mongoTemplate.remove(query, COLLECTION_NAME);
        if (deleteResult.getDeletedCount() > 0) {
            log.info("删除Feed成功，id:{}", id);
        } else {
            log.info("删除Feed失败，id:{}", id);
        }
    }
