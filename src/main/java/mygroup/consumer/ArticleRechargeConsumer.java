...
@Slf4j
@Component
@RabbitListener(queues = QueueConstant.Article_Recharge)
public class ArticleRechargeConsumer {
    ...
    public void process(String jsonData) {
        ...
        try {
            ...
            articleDoc = mongoTemplate.findOne(queryAuthor, ArticleDoc.class, COLLECTION_NAME);
            ...
            // ====================================== 给作者新增经济处理
            UserInfo userInfo = userInfoMapper.selectById(starTransfer.getReceiverId());
            if (null == userInfo) {
                log.error("用户不存在");
                throw new BisException("用户不存在");
            }
            ...
            BigDecimal economyResult = defaultVal.add(BigDecimal.ONE);
            ...
            // ====================================== 给发送者扣除经济处理
            userInfo = userInfoMapper.selectById(starTransfer.getSenderId());
            if (null == userInfo) {
                log.error("用户不存在");
                throw new BisException("用户不存在");
            }
            ...
            economyResult = defaultVal.subtract(BigDecimal.ONE);
            userInfo.setEconomy(economyResult);
            ...
            if (null != starTransfer) {
                if (starTransfer.isSenderSent() && starTransfer.isReceiverReceived()) {
                    ...
                    if (result.getMatchedCount() <= 0) {
                        ...
                        log.info("用户[{}]给文章[{}]充电了", userId, docId);
                        ...
                        if (senderUserId != null) {
                            ...
                            ntf.setContent(String.format("%s %s了你的主题 %s", senderAtLink, UserMsgType.REWARD, topicLink));
