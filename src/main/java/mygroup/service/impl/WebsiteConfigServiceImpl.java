...
import mygroup.mapper.WebsiteConfigMapper;
...
public class WebsiteConfigServiceImpl extends ServiceImpl<WebsiteConfigMapper, WebsiteConfig> implements IWebsiteConfigService {
    ...
    public void setCache() {
        this.baseMapper.selectList(null).forEach(websiteConfig -> {
            WebsiteConfigUtil.setWebsiteConfig(websiteConfig.getConfigKey(), websiteConfig.getConfigValue());
        });
    }

    @Override
    public void add(WebsiteConfig websiteConfig) {
        if (this.baseMapper.insert(websiteConfig) > 0) {
            WebsiteConfigUtil.setWebsiteConfig(websiteConfig.getConfigKey(), websiteConfig.getConfigValue());
        }
    }
